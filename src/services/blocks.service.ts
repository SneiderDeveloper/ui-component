...
const baseUrl = 'https://ibuilder.ozonohosting.com/'
...
export const getBlockFetcher = (url: string) =>
  axios.get(`${baseUrl}${url}`, { headers: { 'Authorization': 'Bearer ' + token } }).then(res => res.data.data)

export const getDataFetcher = (url: string, params= {}) =>
  axios.get(`${baseUrl}${url}`, { ...params }).then(res => res.data.data)
